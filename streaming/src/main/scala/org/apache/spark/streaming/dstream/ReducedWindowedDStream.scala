...
class ReducedWindowedDStream[K: ClassTag, V: ClassTag](
  ...
  override def compute(validTime: Time): Option[RDD[(K, V)]] = {
    ...
    val currentWindow: Interval = new Interval(currentTime - windowDuration + parent.slideDuration,
      currentTime)
    val previousWindow: Interval = currentWindow - slideDuration
    ...
    val oldRDDs: Seq[RDD[(K, V)]] = reducedStream.slice(previousWindow.beginTime, currentWindow.beginTime - parent.slideDuration)
    // Get the RDDs of the reduced values in "old time steps"
    logDebug("# old RDDs = " + oldRDDs.size)

    // Get the RDDs of the reduced values in "new time steps"
    val newRDDs: Seq[RDD[(K, V)]] =
